...
import csSurfaceFunc from './simple-surface-func.wgsl';
import csSurface from './simple-surface-comp.wgsl';
...
const run = async () => {
    ...
    gui.add(params, 'surfaceType', ['sinc','peaks', 'poles']).onChange((val:string) => {
        if(val === 'sinc') funcSelection = 0;
        else if(val === 'peaks') funcSelection = 1;
        else funcSelection = 2;
    });
    ...
    folder.add(params, 'resolution', 16, 2048, 8).onChange(() => {      
        resolutionChanged = true;
    });
    
    folder.add(params, 'colormap', [
        'autumn', 'black', 'blue', 'bone', 'cool', 'copper', 'cyan', 'fuchsia', 'green', 'greys', 'hsv', 'hot', 
